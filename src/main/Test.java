...
public class Test {
	...
	// Demo
	P5Canvas bouncer = new P5Canvas();
	MainController bouncer_controller = new MainController(bouncer);
	...
	private void initialize() {
		...
		frmConnectedChemistrySimulations.getContentPane().setLayout(new MigLayout("", "[fill][grow,fill][100px,grow]", "[grow,fill][]"));
		...
		demoPanel.setLayout(new MigLayout("", "[grow,fill]", "[grow,fill]"));
		demoPanel.add(bouncer, "cell 0 0,alignx left,aligny top");
		
		JPanel panel = new JPanel();
		panel.setBackground(Color.GRAY);
		frmConnectedChemistrySimulations.getContentPane().add(panel, "cell 2 0,grow");
		...
		bouncer.setLayout(new CardLayout(0, 0));
		
		JButton button = new JButton("New button");
		bouncer.add(button, "name_1304959411342089000");
